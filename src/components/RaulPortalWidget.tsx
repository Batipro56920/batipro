...
const WELCOME_MESSAGE: RaulChatMessage = {
  role: "assistant",
  content: "Salut, je suis Raul. Pose-moi une question sur ta tâche, un imprévu, ou comment déclarer un matériau.",
};
...
export default function RaulPortalWidget({ token, chantierId }: { token: string; chantierId: string | null }) {
  ...
  return (
    <div className="fixed bottom-24 right-3 z-40 max-w-[calc(100vw-1.5rem)]">
      {open ? (
        <section className="mb-3 flex h-[min(480px,calc(100dvh-11rem))] w-[min(340px,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-950/20">
          <header className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-950 px-4 py-3 text-white">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="truncate text-xs text-slate-300">Assistant chantier</div>
              </div>
            </div>
            <button
              type="button"
