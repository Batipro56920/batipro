...
type RaulChatMessage = { role: "user" | "assistant"; content: string; attachments?: Array<{ name: string; mime_type: string }> };
const WELCOME_MESSAGE: RaulChatMessage = { role: "assistant", content: "Salut, je suis Raul. Pose-moi une question sur ta tâche, un imprévu, ou envoie-moi une photo du chantier à analyser." };
...
export default function RaulPortalWidget({ token, chantierId }: { token: string; chantierId: string | null }) {
  const [open, setOpen] = useState(false); const [input, setInput] = useState(""); const [messages, setMessages] = useState<RaulChatMessage[]>([WELCOME_MESSAGE]); const [attachments, setAttachments] = useState<RaulAttachment[]>([]); const [preparingAttachment, setPreparingAttachment] = useState(false); const [sending, setSending] = useState(false); const [error, setError] = useState<string | null>(null);
  ...
  async function addFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    setPreparingAttachment(true); setError(null);
    try {
  ...
  async function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    if (preparingAttachment) { setError("Attends une seconde : la photo est encore en cours de préparation."); return; }
    const content = input.trim(); if ((!content && attachments.length === 0) || sending) return;
    const sentAttachments = [...attachments];
    const hasImage = sentAttachments.some((item) => item.mime_type.startsWith("image/"));
    const displayContent = content || (hasImage ? "Analyse cette photo et décris précisément ce que tu vois avant de me conseiller." : "Analyse ce fichier.");
    ...
    setMessages(nextMessages); setInput(""); setAttachments([]); setSending(true); setError(null);
    try {
      const { data, error: invokeError } = await supabase.functions.invoke("raul-chatbot", { body: { message: displayContent, history: nextMessages.slice(-10), attachments: sentAttachments, token, chantier_id: chantierId } });
      if (invokeError) throw invokeError;
      const reply = String((data as { reply?: string } | null)?.reply ?? "").trim(); if (!reply) throw new Error("Réponse vide de Raul.");
  ...
  return <div className="fixed bottom-24 right-3 z-40 max-w-[calc(100vw-1.5rem)]">
    {open ? <section className="mb-3 flex h-[min(480px,calc(100dvh-11rem))] w-[min(340px,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-950/20">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-4">{messages.map((message, index) => { const fromUser = message.role === "user"; return <div key={`${message.role}-${index}`} className={fromUser ? "flex justify-end" : "flex justify-start"}><div className={["max-w-[85%] rounded-2xl px-3 py-2 text-sm leading-6", fromUser ? "bg-blue-600 text-white" : "border border-slate-200 bg-white text-slate-800"].join(" ")}>{message.attachments?.map((item, i) => <div key={`${item.name}-${i}`} className="mb-1 flex items-center gap-1 rounded-lg bg-black/10 px-2 py-1 text-xs"><Camera className="h-3 w-3" /><span className="truncate">Photo jointe : {item.name}</span></div>)}{message.content}</div></div>; })}{sending ? <div className="text-xs text-slate-500">Raul analyse la demande...</div> : null}{error ? <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">{error}</div> : null}</div>
      <form onSubmit={sendMessage} className="border-t border-slate-200 bg-white p-3">
        {preparingAttachment ? <div className="mb-2 rounded-xl bg-blue-50 px-3 py-2 text-xs font-medium text-blue-700">Préparation de la photo…</div> : null}
        {attachments.length ? <div className="mb-2 flex flex-wrap gap-2">{attachments.map((item, index) => <span key={`${item.name}-${index}`} className="flex max-w-full items-center gap-1 rounded-lg border border-green-200 bg-green-50 px-2 py-1 text-xs font-medium text-green-800">{item.mime_type.startsWith("image/") ? <Camera className="h-3 w-3" /> : <FileText className="h-3 w-3" />}<span className="max-w-44 truncate">✓ {item.name}</span><button type="button" onClick={() => setAttachments((prev) => prev.filter((_, i) => i !== index))}><X className="h-3 w-3" /></button></span>)}</div> : null}
        <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={addFiles} /><input ref={fileInputRef} type="file" accept={RAUL_FILE_ACCEPT} multiple className="hidden" onChange={addFiles} />
