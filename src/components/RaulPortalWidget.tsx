...
export default function RaulPortalWidget({ token, chantierId }: { token: string; chantierId: string | null }) {
  ...
  async function sendMessage(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (preparingAttachment) { setError("Attends une seconde : la photo est encore en cours de préparation."); return; }
    const content = input.trim(); if ((!content && attachments.length === 0) || sending) return;
    const sentAttachments = [...attachments]; const hasImage = sentAttachments.some((item) => item.mime_type.startsWith("image/"));
    ...
    try {
      const { data, error: invokeError } = await supabase.functions.invoke("raul-chatbot", { body: { message: displayContent, history: nextMessages.slice(-10).map(({ role, content }) => ({ role, content })), attachments: sentAttachments, token, chantier_id: chantierId } });
      if (invokeError) throw invokeError;
      const payload = data as { reply?: string; generated_image?: string | null } | null;
