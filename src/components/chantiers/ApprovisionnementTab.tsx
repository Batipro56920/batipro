...
export default function ApprovisionnementTab({ chantierId, tasks, zones }: ApprovisionnementTabProps) {
  ...
  async function generateMaterialsFromTasks() {
    ...
    try {
      ...
      if (!preparation.schemaReady) {
        setMaterialsError("Migration préparation avancée non appliquée sur Supabase.");
        return;
      }
      ...
      for (const task of tasks) {
        ...
        for (const material of estimate.materials) {
          ...
          if (existing) {
            ...
            existing.estimated_cost_ht =
              (existing.estimated_cost_ht ?? 0) + (material.estimated_purchase_cost_ht ?? 0);
      ...
      if (aggregated.size === 0) {
        setMaterialsError(
          "Aucun ratio matériau compatible trouvé sur les tâches de ce chantier (templates sans ratios, ou unités incompatibles).",
        );
      }
    } catch (err: any) {
      setMaterialsError(err?.message ?? "Erreur génération liste matériaux.");
    } finally {
  ...
  function addManualMaterialLine() {
    ...
    if (!name) return;
    const quantity = parseAmount(manualMaterialQuantity) || 1;
    setMaterialLines((current) => [
      ...current,
      {
        id: `manual-${Date.now()}`,
  ...
  async function createPurchaseRequestsFromMaterials() {
    ...
    try {
      for (const line of materialLines) {
        const saved = await createChantierPurchaseRequest({
          ...
          titre: line.material_name,
          quantite: line.quantity,
          unite: line.unit || null,
          cout_prevu_ht: line.estimated_cost_ht ?? 0,
          statut_commande: "a_commander",
          commentaire:
            line.source === "auto"
              ? `Généré depuis ${line.taskCount} tâche(s) du chantier (ratios bibliothèque).`
              : "Ajouté manuellement.",
        });
        setRequests((current) => [saved, ...current]);
        await logPurchaseAction("created", saved, "Demande approvisionnement créée depuis la liste matériaux", {
          titre: saved.titre,
          source: line.source,
        });
      }
      ...
      setMaterialsError(err?.message ?? "Erreur création des demandes fournisseur.");
    } finally {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-3xl border border-blue-200 bg-blue-50/60 p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="text-sm font-semibold text-slate-900">Liste de matériaux</div>
            <div className="text-sm text-slate-600">
              Calculée à partir des ratios matériaux (bibliothèque de tâches) × quantités des tâches de ce chantier.
              Ajuste ou complète avant de créer les demandes.
            </div>
          </div>
          <button
            type="button"
            ...
            className="shrink-0 rounded-2xl border border-blue-300 bg-white px-4 py-2 text-sm font-medium text-blue-800 hover:bg-blue-100 disabled:opacity-60"
          >
            {materialsGenerating ? "Calcul..." : "Calculer depuis les tâches"}
          </button>
        </div>
              ...
              >
                <div className="min-w-[160px] flex-1">
                  ...
                  <div className="text-xs text-slate-500">
                    {line.source === "auto" ? `Auto · ${line.taskCount} tâche(s)` : "Ajout manuel"}
                    {line.estimated_cost_ht !== null ? ` · ~${formatMoney(line.estimated_cost_ht)} HT` : ""}
                  </div>
                </div>
                <input
                  value={line.quantity}
        ...
        <div className="mt-4 flex flex-wrap items-end gap-3 border-t border-blue-200/60 pt-4">
          ...
          <button
            type="button"
            onClick={() => void createPurchaseRequestsFromMaterials()}
          ...
          >
            {materialsCreating
              ? "Création..."
              : `Créer bon de commande (${materialLines.length} ligne${materialLines.length > 1 ? "s" : ""})`}
          </button>
        </div>
      </div>
