...
import { ChevronLeft, ChevronRight, Copy, GripVertical, Plus, Scissors, Trash2, X } from "lucide-react";
...
export default function PlanningTab({ chantierId, chantierName, intervenants }: Props) {
  ...
  const plannedTasks = useMemo(() => tasks.filter((task) => task.date_debut), [tasks]);
  const backlogTasks = useMemo(() => {
    const query = backlogQuery.trim().toLowerCase();
    return tasks
      .filter((task) => !task.date_debut)
  ...
  }, [assigneeFilter, backlogQuery, tasks]);
  ...
  return (
    <DndContext onDragEnd={(event) => void handleDragEnd(event)}>
      <div className="flex flex-col gap-4 xl:grid xl:grid-cols-[18rem_minmax(0,1fr)_22rem] xl:items-start">
        <aside className="space-y-4 xl:sticky xl:top-4">
          <div
            ref={backlogDrop.setNodeRef}
            className={[
              "rounded-3xl border border-slate-200 bg-white p-4 shadow-sm",
              backlogDrop.isOver ? "border-blue-400 bg-blue-50/60" : "",
            ].join(" ")}
          >
            <div className="mb-3 flex items-center justify-between gap-2">
              <div>
                <div className="text-sm font-semibold text-slate-900">Taches non planifiees</div>
                <div className="text-xs text-slate-500">Drag & drop vers un jour</div>
              </div>
            ...
            <div className="space-y-2">
              <input className={inputClass()} placeholder="Rechercher..." value={backlogQuery} onChange={(e) => setBacklogQuery(e.target.value)} />
            ...
            <div className="mt-3 space-y-2">
              {backlogTasks.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">Aucune tache en backlog.</div>
              ) : (
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold text-slate-900">Synthese periode</div>
                <div className="text-xs text-slate-500">{rangeSummary.totalHours}h prevues</div>
              </div>
              <button type="button" className={buttonClass()} onClick={() => setShowSettings((current) => !current)}>
                {showSettings ? "Masquer" : "Reglages"}
              </button>
            </div>
            <div className="space-y-2 text-sm text-slate-600">
              {rangeSummary.totalsByIntervenant.slice(0, 4).map(([intervenantId, hours]) => (
                <div key={intervenantId} className="flex items-center justify-between gap-2">
                  <span className="truncate">{intervenantsById.get(intervenantId)?.nom ?? "Intervenant"}</span>
                  <span className="font-medium text-slate-900">{hours.toFixed(1)}h</span>
                </div>
              ))}
            </div>
            {showSettings ? (
        ...
        <main className="space-y-4">
              ...
              <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-3">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button type="button" className={buttonClass("danger")} onClick={() => void deletePlanningCalendarTasks(selectedTaskIds).then(() => loadAll(true)).then(() => setSelectedTaskIds([])).catch((err) => setError(err?.message ?? "Erreur suppression."))}>
          ...
          {view === "month" ? (
            ...
            <section className={view === "day" ? "grid gap-3" : "grid gap-3 lg:grid-cols-7"}>
              {visibleDays.map((day) => (
        ...
        <aside className={[
          "rounded-3xl border border-slate-200 bg-white p-4 shadow-sm",
          drawer ? "block" : "hidden xl:block",
          drawer ? "fixed inset-x-0 bottom-0 z-50 max-h-[85vh] overflow-y-auto rounded-t-3xl xl:sticky xl:top-4 xl:max-h-none xl:rounded-3xl" : "",
        ].join(" ")}>
          <div className="mb-4 flex items-center justify-between gap-2 border-b border-slate-100 pb-3">
            <div>
              <div className="text-sm font-semibold text-slate-900">
                {drawer?.mode === "task" ? "Editer la tache" : drawer?.mode === "create" ? "Nouvelle tache" : drawer?.mode === "day" ? "Jour" : "Apercu"}
              </div>
              <div className="text-xs text-slate-500">
                {drawer?.mode === "day" ? formatDisplayDate(drawer.day) : chantier?.nom ?? chantierName ?? "Planning"}
              </div>
            </div>
            {drawer ? <button type="button" className={buttonClass()} onClick={() => setDrawer(null)}><X className="h-4 w-4" /></button> : null}
          </div>
          {drawer?.mode === "day" ? (
            <div className="space-y-3">
              <button type="button" className={buttonClass("primary")} onClick={() => setDrawer({ mode: "create", startDate: drawer.day })}>
                <Plus className="mr-1 inline h-4 w-4" />Ajouter dans ce jour
              </button>
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <label className="block">
                  <span className="mb-1 block text-xs font-medium text-slate-500">Debut</span>
            ...
            <div className="rounded-2xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">
              Selectionne une tache ou un jour pour editer le planning.
            </div>
          )}
        </aside>
      </div>
