...
type Props = {
  ...
  intervenants: DrawerIntervenant[];
  selectedIntervenantIds: string[];
  onIntervenantSelectionChange: (next: string[]) => void;
  shareWithAllIntervenants: boolean;
  onShareWithAllIntervenantsChange: (next: boolean) => void;
  query: string;
...
export default function TaskDocumentsDrawer({
  ...
  onSelectionChange,
  intervenants,
  selectedIntervenantIds,
  onIntervenantSelectionChange,
  shareWithAllIntervenants,
  onShareWithAllIntervenantsChange,
  query,
  onQueryChange,
  onClose,
  onSave,
  ...
  const selectableIntervenants = useMemo(
    () => intervenants.filter((intervenant) => Boolean(intervenant.id)),
    [intervenants],
  );

  const allIntervenantIds = useMemo(
    () => selectableIntervenants.map((intervenant) => intervenant.id),
    [selectableIntervenants],
  );
        ...
        <div className="flex-1 overflow-hidden flex">
          <div className="w-[55%] border-r overflow-auto p-4 space-y-3">
            <div className="rounded-xl border bg-slate-50 p-3 space-y-2">
              <div className="text-xs font-semibold text-slate-700">Autorisations intervenants</div>
              <label className="flex items-center gap-2 text-xs text-slate-600">
                <input
                  type="checkbox"
                  checked={shareWithAllIntervenants}
                  onChange={(e) => {
                    const checked = e.target.checked;
                    onShareWithAllIntervenantsChange(checked);
                    onIntervenantSelectionChange(checked ? allIntervenantIds : []);
                  }}
                />
                Tous les intervenants du chantier
              </label>
              {!shareWithAllIntervenants && (
                <div className="max-h-36 overflow-auto rounded-lg border bg-white p-2">
                  {selectableIntervenants.length === 0 ? (
                    <div className="text-xs text-slate-500">Aucun intervenant disponible.</div>
                  ) : (
                    <div className="space-y-1">
                      {selectableIntervenants.map((intervenant) => {
                        const checked = selectedIntervenantIds.includes(intervenant.id);
                        return (
                          <label
                            key={intervenant.id}
                          ...
                          >
                            <input
                              ...
                              onChange={() => {
                                if (checked) {
                                  onIntervenantSelectionChange(
                                    selectedIntervenantIds.filter((id) => id !== intervenant.id),
                                  );
                                } else {
                                  onIntervenantSelectionChange([...selectedIntervenantIds, intervenant.id]);
                                }
                              }}
                            />
