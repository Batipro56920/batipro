...
export default function PilotageTab({ chantierId, tasks, zones }: PilotageTabProps) {
  ...
  useEffect(() => {
    if (!targetedChangeOrder) return;
    setFilterType(normalizeChangeOrderType(targetedChangeOrder.type_ecart));
  }, [targetedChangeOrder]);
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-base font-semibold text-slate-950">Imprevus / Travaux supplementaires</h2>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">Imprevus</div><div className="mt-1.5 text-base font-semibold text-slate-950">{imprevus.length}</div><div className="mt-0.5 text-xs text-slate-500">Perte estimee {formatMoney(perteBudget)}</div></div>
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">Travaux supplementaires</div><div className="mt-1.5 text-base font-semibold text-slate-950">{tsRows.length}</div><div className="mt-0.5 text-xs text-slate-500">Potentiel facture {formatMoney(tsValidesHt)}</div></div>
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">Regle</div><div className="mt-1.5 text-sm font-semibold text-slate-950">Un TS ne part pas en execution sans validation client.</div></div>
      </section>
      ...
      {targetedChangeOrderId ? (
        <div className={[
          ...
          <div className="font-semibold">
            {targetedChangeOrderMissing ? "Imprevu / TS introuvable" : "Imprevu / TS cible depuis la recherche globale"}
          </div>
          <div className={targetedChangeOrderMissing ? "mt-1 text-amber-800" : "mt-1 text-blue-800"}>
            {targetedChangeOrderMissing
              ...
              : `${targetedChangeOrder?.titre ?? "L'element"} est affiche dans le suivi ci-dessous avec son type exact.`}
          </div>
        </div>
      ) : null}
