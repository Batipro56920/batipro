...
function BacklogCard({
  ...
  return (
    <div
      ...
      style={style}
      className={[
        "rounded-[1.6rem] bg-white/95 p-4 shadow-[0_14px_30px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/70 transition",
    ...
    >
      <div className="flex items-start gap-3">
        <button type="button" className="mt-0.5 shrink-0 cursor-grab rounded-2xl p-2 text-slate-400 hover:bg-slate-100" {...drag.listeners} {...drag.attributes}>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 text-xs text-slate-500">
        <button type="button" className={buttonClass("primary")} onClick={onOpen}>Ouvrir</button>
...
function PlanningCell({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={[
        "relative min-h-[120px] border-l border-slate-200/50 bg-white/70 text-left transition first:border-l-0 hover:bg-white",
        overloaded ? "bg-amber-50/70" : "",
        isOver ? "bg-blue-50/80 shadow-inner" : "",
      ].join(" ")}
    >
      <div className="pointer-events-none absolute bottom-3 right-3 text-[11px] font-semibold text-slate-400">{loadHours > 0 ? formatHours(loadHours) : ""}</div>
    </button>
  );
}
...
function PlanningBlockCard({
  ...
  return (
    <button
      ...
      className={["absolute z-10 rounded-[1.35rem] px-4 py-3 text-left shadow-[0_10px_24px_rgba(15,23,42,0.08)] ring-1 ring-black/5 transition hover:-translate-y-0.5 hover:shadow-[0_16px_30px_rgba(15,23,42,0.14)]", selected ? "ring-2 ring-slate-900/15" : ""].join(" ")}
    >
      <div className="line-clamp-2 text-sm font-semibold">{block.title}</div>
      <div className="mt-2 text-xs font-semibold opacity-75">{formatHours(block.plannedHours)}</div>
    </button>
  );
}

export default function PlanningBoard({ chantierId, chantierName, intervenants }: Props) {
  ...
  const boardBlocksByRow = useMemo(() => {
    ...
    for (const segment of visibleSegments) {
      ...
      list.push({
        ...
        color: segment.intervenant_id ? getIntervenantColor(segment.intervenant_id) : getIntervenantColor("sans-affectation"),
  ...
  return (
    <DndContext onDragEnd={(event) => void onDragEnd(event)}>
      <div className="space-y-4">
        ...
        <section className="rounded-[2rem] bg-white/80 p-5 shadow-[0_24px_60px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/70 backdrop-blur">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-2 text-sm text-slate-500">Un board custom leger en React et dnd-kit. Les taches deviennent des cartes simples a deplacer entre jours et intervenants.</div>
            </div>

            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center rounded-2xl bg-slate-50 p-1 ring-1 ring-slate-200/70">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-sm">
            <div className="rounded-full bg-white px-4 py-2 text-slate-600 shadow-sm ring-1 ring-slate-200/70">
              <span className="font-semibold text-slate-900">{taskCards.filter((item) => item.planningState === "a_planifier").length}</span> taches a planifier
            </div>
            <div className="rounded-full bg-white px-4 py-2 text-slate-600 shadow-sm ring-1 ring-slate-200/70">
              <span className="font-semibold text-slate-900">{visibleWeekBlocks}</span> cartes visibles
            </div>
            <div className="rounded-full bg-white px-4 py-2 text-slate-600 shadow-sm ring-1 ring-slate-200/70">
              Charge <span className="font-semibold text-slate-900">{formatHours(weeklyHours)}</span>
            </div>
            <div className={["rounded-full px-4 py-2 shadow-sm ring-1", alerts.length ? "bg-rose-50 text-rose-700 ring-rose-200" : "bg-white text-slate-600 ring-slate-200/70"].join(" ")}>
        ...
        <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
          <aside className="rounded-[2rem] bg-white/75 p-4 shadow-[0_20px_48px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/60 backdrop-blur">
            ...
            <div className="mt-4 max-h-[72vh] space-y-3 overflow-auto pr-1">
              {filteredTasks.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-200 p-4 text-sm text-slate-500">Aucune tache avec ces filtres.</div> : filteredTasks.map((item) => (
                <BacklogCard key={item.task.id} item={item} settings={settings} selected={selectedTaskId === item.task.id && !selectedSegmentId} onOpen={() => openTask(item.task.id)} />
              ))}
            </div>
          </aside>
          ...
          <section className="rounded-[2rem] bg-white/75 p-4 shadow-[0_20px_48px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/60 backdrop-blur">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                <div className="text-sm font-semibold text-slate-900">Planning equipe</div>
                <div className="text-xs text-slate-500">Colonnes = jours, lignes = intervenants, cartes = blocs de taches.</div>
              </div>
              <div className="flex flex-wrap gap-2">
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[1120px]">
                <div className="grid rounded-t-[1.75rem] bg-slate-50/90 ring-1 ring-slate-200/70" style={{ gridTemplateColumns: `230px repeat(${visibleDays.length}, minmax(0,1fr))` }}>
                  <div className="px-4 py-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Intervenants</div>
                  {visibleDays.map((day) => (
                    <div key={day} className="border-l border-slate-200/50 px-4 py-4">
                      <div className="text-sm font-semibold text-slate-900">{formatDayLabel(day, locale)}</div>
                      <div className="mt-1 text-xs font-medium text-slate-500">{dayTotals.get(day) ? formatHours(dayTotals.get(day) ?? 0) : "A planifier"}</div>
                    </div>
                  ))}
                </div>

                <div className="overflow-hidden rounded-b-[1.75rem] bg-white/75 ring-1 ring-slate-200/70">
                  {rows.map((row) => {
                    const rowBlocks = boardBlocksByRow.get(row.id) ?? [];
                    const lanesCount = Math.max(1, ...rowBlocks.map((block) => block.lane + 1));
                    const rowHeight = Math.max(120, lanesCount * CELL_HEIGHT);
                    return (
                      <div key={row.id} className="grid grid-cols-[230px_minmax(0,1fr)] border-b border-slate-100 last:border-b-0">
                        ...
                        <div className="relative" style={{ minHeight: rowHeight }}>
                          <div className="grid h-full" style={{ gridTemplateColumns: `repeat(${visibleDays.length}, minmax(0,1fr))` }}>
                            {visibleDays.map((day) => <PlanningCell key={`${row.id}:${day}`} id={`cell:${row.id}:${day}`} loadHours={rowLoadByCell.get(`${row.id}:${day}`) ?? 0} overloaded={(rowLoadByCell.get(`${row.id}:${day}`) ?? 0) > settings.hoursPerDay} onClick={() => prefillFromCell(day, row.id)} />)}
                          </div>
                          <div className="pointer-events-none absolute inset-0">{rowBlocks.map((block) => <div key={block.id} className="pointer-events-auto"><PlanningBlockCard block={block} count={visibleDays.length} selected={selectedSegmentId === block.segment.id} onSelect={() => openSegment(block.segment.id)} /></div>)}</div>
                        </div>
                      </div>
                    );
                  })}
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-1 text-xs text-slate-500">{selectedTask ? `${selectedTask.lot ?? selectedTask.corps_etat ?? "Sans lot"} - ${formatHours(selectedTaskSummary?.remainingHours ?? 0)} restantes` : "Selectionne une tache a planifier."}</div>
              </div>
