...
export default function TaskTemplateDrawer({
  ...
  function applyProductToMaterial(index: number, productId: string) {
    ...
    updateMaterialDraft(index, {
      ...
      source_unit: hint.sourceUnit ?? product.unit,
      ratio_unit: hint.ratioUnit ?? product.unit,
      ...(hint.quantity !== null ? { ratio_quantity: toField(hint.quantity) } : {}),
  ...
  async function handleGenerateWithCoco() {
    // Garde-fous et feedback repris de l'implementation main.
    setLocalError(null);
    setCocoError(null);
    setCocoMessage(null);
    if (!titre.trim()) {
    ...
    try {
      ...
      if (result.usedFallback) {
        setCocoError(`Coco IA indisponible. Préparation métier locale appliquée : ${result.errorMessage ?? "erreur inconnue"}`);
      } else {
        setCocoMessage(`Suggestions Coco appliquées.${missing}`);
      }
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-0 top-0 h-screen w-[46vw] max-w-[860px] min-w-[360px] bg-white border-l shadow-xl flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          ...
          <label className="block space-y-1">
            <div className="text-xs text-slate-600">Contexte métier</div>
            <textarea
            ...
            />
            <div className="text-[11px] text-slate-500">Transmis à Coco pour orienter le mode opératoire généré.</div>
          </label>
            ...
            <div className="space-y-4 rounded-2xl border border-blue-200 bg-blue-50/40 p-4">
              ...
              <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="text-sm font-semibold text-slate-900">Assistant Coco - resultat detaille</div>
                    <div className="mt-1 text-xs text-slate-500">
                      Coco exploite la designation, l'unite, le contexte metier, les produits lies, les ratios, la main d'oeuvre et les frais avant de remplir les blocs techniques.
                      La generation se lance depuis le bloc « Coco · préparation métier » en haut du formulaire.
                    </div>
                  </div>
                </div>
...
function profileToInput(profile: TaskTemplateLotProfile): TaskTemplateLotProfileInput {
  return {
    ...
    defaultQuoteVisible: profile.defaultQuoteVisible,
    defaultChantierVisible: profile.defaultChantierVisible,
        ...
        <div className="grid min-h-0 flex-1 md:grid-cols-[260px_minmax(0,1fr)]">
          ...
          <div className="min-h-0 overflow-y-auto p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <label className="flex items-center gap-3 rounded-xl border border-sky-200 bg-white px-3 py-2 text-sm text-slate-700">
                <input
                  type="checkbox"
                  checked={draft.defaultQuoteVisible}
                  onChange={(e) => patch({ defaultQuoteVisible: e.target.checked })}
                />
                <span>Visible dans les devis par défaut</span>
              </label>
              <label className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-white px-3 py-2 text-sm text-slate-700">
                <input
                  type="checkbox"
                  checked={draft.defaultChantierVisible}
                  onChange={(e) => patch({ defaultChantierVisible: e.target.checked })}
                />
                <span>Visible côté chantier par défaut</span>
              </label>
            </div>
          </div>
        </div>
