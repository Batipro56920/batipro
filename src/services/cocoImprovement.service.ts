...
function templateSimilarity(task: Row, template: Row) {
  const taskTokens = new Set(normalizeComparableText(task.titre).split(" ").filter((token) => token.length > 3));
  const templateTokens = normalizeComparableText(template.titre).split(" ").filter((token) => token.length > 3);
