...
export function CommunicationApprovals() {
  ...
  async function decide(decision: "approved" | "changes_requested") {
    ...
    try {
      ...
      setMessage(decision === "approved" ? "Versions sélectionnées validées." : "Corrections demandées."); await reload();
  ...
  return <section className="grid overflow-hidden rounded-2xl border border-subtle bg-surface shadow-sm lg:grid-cols-[320px_1fr]">
    ...
    <main className="min-h-96 p-5">{item ? <><header><p className="text-xs font-semibold uppercase tracking-wide text-primary">{item.campaign_title}</p><h2 className="mt-1 text-xl font-semibold text-ink">{item.title}</h2><p className="mt-2 whitespace-pre-wrap text-sm text-muted">{item.content}</p></header>
      ...
      <label className="mt-5 block text-sm font-medium text-ink">Commentaire pour l’équipe<textarea value={note} onChange={(event) => setNote(event.target.value)} rows={3} className={`${inputClass} mt-1`} placeholder="Préciser ce qu’il faut modifier…" /></label>{message ? <p className="mt-3 rounded-xl bg-interactive p-3 text-sm text-ink">{message}</p> : null}
    ...
    </> : <div className="flex h-full min-h-80 flex-col items-center justify-center text-center"><CheckCircle2 className="h-10 w-10 text-success" /><h2 className="mt-3 font-semibold text-ink">Tout est validé</h2><p className="mt-1 text-sm text-muted">Les nouvelles demandes de validation apparaîtront ici.</p></div>}</main>
  </section>;
}
