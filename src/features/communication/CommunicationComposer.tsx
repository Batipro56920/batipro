...
/**
 * Brouillon local du compositeur.
 *
 * Rediger une publication prend du temps : texte commun, variante par reseau,
 * lien, date. Rien de tout cela n'existait ailleurs que dans la memoire de
 * l'onglet — un rechargement, une coupure, un onglet ferme par erreur, et la
 * saisie repartait de zero. Le brouillon est donc conserve sur le poste, et
 * efface des que la publication est enregistree pour de bon.
 *
 * Les fichiers joints n'en font pas partie : un fichier choisi n'existe que
 * dans l'onglet qui l'a ouvert, il est a reselectionner.
 */
const DRAFT_STORAGE_KEY = "batipro.communication.composer.v1";
...
function readComposerDraft(): Partial<ComposerDraft> {
  try {
    ...
    // Stockage refuse ou brouillon illisible : on repart a vide plutot que de
    // casser l'ecran pour un confort de saisie.
    return {};
  }
}

function writeComposerDraft(draft: ComposerDraft) {
  try {
    window.localStorage.setItem(DRAFT_STORAGE_KEY, JSON.stringify(draft));
  } catch {
    // Sans stockage, on perd seulement la reprise apres rechargement.
  }
}
...
function clearComposerDraft() {
  try {
    ...
    // Rien a faire : le brouillon sera ecrase a la prochaine saisie.
  }
}
...
export function CommunicationComposer({ campaigns, chantiers, onSaved }: Props) {
  // Lu une seule fois, au montage : ensuite c'est la saisie qui fait foi.
  const [restored] = useState(readComposerDraft);
  const [draftNoticeDismissed, setDraftNoticeDismissed] = useState(false);
  const [accounts, setAccounts] = useState<SocialAccount[]>([]);
  const [campaignId, setCampaignId] = useState(restored.campaignId ?? campaigns[0]?.id ?? "");
  const [chantierId, setChantierId] = useState(restored.chantierId ?? "");
  const [title, setTitle] = useState(restored.title ?? "");
  const [baseContent, setBaseContent] = useState(restored.baseContent ?? "");
  const [selectedNetworks, setSelectedNetworks] = useState<SocialNetwork[]>(restored.selectedNetworks ?? ["facebook", "instagram"]);
  const [variants, setVariants] = useState<Partial<Record<SocialNetwork, string>>>(restored.variants ?? {});
  ...
  const [destinationUrl, setDestinationUrl] = useState(restored.destinationUrl ?? "");
  const [trackingEnabled, setTrackingEnabled] = useState(restored.trackingEnabled ?? true);
  ...
  useEffect(() => {
    writeComposerDraft({ campaignId, chantierId, title, baseContent, selectedNetworks, variants, scheduledAt, destinationUrl, trackingEnabled });
  }, [campaignId, chantierId, title, baseContent, selectedNetworks, variants, scheduledAt, destinationUrl, trackingEnabled]);
  useEffect(() => { if (campaigns[0] && !campaigns.some((campaign) => campaign.id === campaignId)) setCampaignId(campaigns[0].id); }, [campaignId, campaigns]);

  const showDraftNotice = !draftNoticeDismissed && Boolean((restored.title ?? "").trim() || (restored.baseContent ?? "").trim());
  ...
  /** Un fichier que le stockage refusera ne doit pas attendre l'enregistrement pour le dire. */
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
    <section className="space-y-5 rounded-2xl border border-subtle bg-surface p-5 shadow-sm">
      ...
      {showDraftNotice ? (
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-subtle bg-interactive px-3 py-2 text-sm text-muted">
          <span>Brouillon repris de votre dernière saisie. Les fichiers joints sont à resélectionner.</span>
          <button type="button" onClick={discardDraft} className="font-semibold text-primary hover:underline">Repartir d’une page vide</button>
        </div>
      ) : null}
      <div className="flex flex-wrap gap-2 rounded-xl bg-interactive p-3"><a href={CANVA_SOCIAL_MEDIA_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-subtle bg-surface px-3 py-2 text-sm font-medium text-ink hover:border-primary"><Sparkles className="h-4 w-4 text-primary"/>Créer un visuel dans Canva<ExternalLink className="h-3 w-3 text-muted"/></a><a href={CAPCUT_EDITOR_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-subtle bg-surface px-3 py-2 text-sm font-medium text-ink hover:border-primary"><Video className="h-4 w-4 text-primary"/>Monter une vidéo dans CapCut<ExternalLink className="h-3 w-3 text-muted"/></a><span className="self-center text-xs text-muted">Exporte puis ajoute plusieurs fichiers ci-dessous. La synchronisation Canva automatique arrive avec OAuth.</span></div>
