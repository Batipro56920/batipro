...
export function ChantiersPlanningView({ rows, onPreview }: { rows: ChantierDerived[]; onPreview: (row: ChantierDerived) => void }) {
  const sorted = [...rows].sort((a, b) => String(getPlanningDate(a) ?? "9999-12-31").localeCompare(String(getPlanningDate(b) ?? "9999-12-31")));
  const lateCount = rows.filter((row) => row.isLate).length;
  const toPlanCount = rows.filter((row) => !getPlanningDate(row)).length;
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-950/[0.03]">
      ...
      <div className="space-y-3">
        {sorted.map((row) => {
          ...
          return (
            <div key={row.id} className="grid w-full gap-3 rounded-2xl border border-slate-200 p-3 text-left transition hover:bg-slate-50 md:grid-cols-[160px_minmax(0,1fr)_180px_120px_auto] md:items-center">
              <div>
