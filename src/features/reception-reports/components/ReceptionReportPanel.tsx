  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-600">PV de réception</div>
          <h2 className="mt-1 text-xl font-semibold text-slate-950">Réception chantier</h2>
          <p className="mt-1 text-sm text-slate-500">Génération, envoi et signature du PV de fin de chantier.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="secondary" onClick={() => setPreviewOpen((open) => !open)}>Prévisualiser</Button>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Field label="Date réception" type="date" value={report.receptionDate} onChange={(receptionDate) => patch({ receptionDate })} />
            <Field label="Référence projet / chantier" value={report.projectReference ?? ""} onChange={(projectReference) => patch({ projectReference: projectReference || null })} />
            <label className={labelClass}>
              ...
              <select className={inputClass} value={report.decision} onChange={(event) => patch({ decision: event.target.value as ReceptionReportDecision })}>
                <option value="without_reserves">Réception sans réserve</option>
                <option value="with_reserves">Réception avec réserves</option>
                <option value="refused">Refus de réception</option>
              </select>
            </label>
            <label className={labelClass}>
              Statut
          ...
          <div className="rounded-2xl border border-slate-200 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <div className="font-semibold text-slate-950">Réserves du PV</div>
                <div className="text-sm text-slate-500">Les réserves importées restent référencées au suivi chantier.</div>
              </div>
              <Button variant="secondary" onClick={addReserve}><Plus className="h-4 w-4" /> Ajouter réserve</Button>
            </div>

            {chantierReservesToImport.length ? (
              <div className="mt-4 rounded-2xl border border-dashed border-blue-200 bg-blue-50 p-3">
                <div className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-blue-700">Réserves chantier disponibles</div>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="mt-4 space-y-3">
              {report.reserves.length ? report.reserves.map((reserve) => (
                <div key={reserve.id} className="grid gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-3 md:grid-cols-[minmax(0,1.5fr)_1fr_1fr_150px_120px_auto]">
                  ...
                  <input className={inputClass} placeholder="Lot concerné" value={reserve.lot ?? ""} onChange={(event) => updateReserve(reserve.id, { lot: event.target.value || null })} />
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Décision</div>
            <div className="mt-2 text-lg font-semibold text-slate-950">{receptionDecisionLabel(report.decision)}</div>
            <div className="mt-1 text-sm text-slate-500">{report.reserves.length} réserve(s) rattachée(s)</div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-4 rounded-xl border border-dashed border-slate-200 p-4 text-center text-xs text-slate-500">
              Signature électronique à connecter au lien client.
            </div>
          </div>
        </aside>
      </div>
...
function syncDocument(report: ReceptionReportRecord, chantier: ChantierRow): BusinessDocument {
  const reserveLines = report.reserves.length
    ? report.reserves.map((reserve, index) => `${index + 1}. ${reserve.description || "Réserve"} - Lot: ${reserve.lot || "-"} - Responsable: ${reserve.responsible || "-"} - Levée prévue: ${reserve.dueDate || "-"} - Statut: ${reserve.status === "lifted" ? "levée" : "ouverte"}`).join("\n")
    : "Aucune réserve.";
  const description = [
    `Chantier: ${chantier.nom}`,
    `Référence: ${report.projectReference || chantier.id}`,
    `Décision: ${receptionDecisionLabel(report.decision)}`,
    `Date de réception: ${report.receptionDate}`,
  ...
  for (const reserve of report.reserves) {
    ...
    const created = await createReserve({
      ...
      description: [
        ...
        reserve.lot ? `Lot concerné: ${reserve.lot}` : null,
        reserve.responsible ? `Responsable: ${reserve.responsible}` : null,
        reserve.dueDate ? `Levée prévue: ${reserve.dueDate}` : null,
        "Origine: PV de réception",
      ].filter(Boolean).join("\n"),
...
function syncReceptionNodes(document: BusinessDocument, content: string) {
  return document.nodes.map((node) => {
    if (node.type !== "section" || !["Réception du chantier", "Reception du chantier"].includes(node.title)) return node;
    return {
      ...node,
