...
export function QuoteLibraryPanel({ templates, onInsertTemplate }: Props) {
  ...
  return (
    <aside className="h-full bg-white p-4">
      ...
      <input className="mt-3 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-300" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher ouvrage..." />
      <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
        {families.map((item) => (
          <button key={item} className={item === family ? "shrink-0 rounded-full bg-slate-900 px-3 py-1 text-xs text-white" : "shrink-0 rounded-full border px-3 py-1 text-xs text-slate-600"} onClick={() => setFamily(item)}>
            {item}
          </button>
        ))}
      </div>
