...
import { AlertTriangle, Building2, Camera, CheckCircle2, Clock3, FileText, Home, ListChecks, Loader2, LogOut, MapPin, MessageSquareWarning, Plus, X } from "lucide-react";
...
type AlertItem = { id: string; title: string; text: string; tone: Tone; kind: string };
...
const USEFUL_DOCUMENT_WORDS = ["notice", "procedure", "procédure", "technique", "fiche", "pose", "materiau", "matériau", "securite", "sécurité"];
...
export default function IntervenantPortalTerrainV2Page() {
  ...
  useEffect(() => {
    ...
    async function loadTasksOverview() {
      const results = await Promise.all(chantiers.map((chantier) => safeLoad(() => intervenantGetTasks(token, chantier.id), [] as IntervenantTask[], "Chargement taches impossible.").then((result) => ({ chantier, result }))));
      if (!alive) return;
      const next: Record<string, IntervenantTask[]> = {};
      results.forEach(({ chantier, result }) => { next[chantier.id] = result.data.sort(sortTasks); });
      setTasksBySite(next);
    }
  ...
  useEffect(() => {
    ...
    async function loadSelectedSite() {
      ...
      if (!alive) return;
      setSiteData({ tasks: tasks.data.sort(sortTasks), documents: documents.data, timeEntries: timeEntries.data, feedbacks: feedbacks.data, reserves: reserves.data, infoRequests: infoRequests.data, consignes: consignes.data });
  ...
  const alerts = useMemo<AlertItem[]>(() => {
    const feedbackAlerts = siteData.feedbacks.filter((feedback) => feedback.status !== "traite" && ["blocage", "anomalie"].includes(feedback.category)).map((feedback) => ({ id: feedback.id, title: feedback.title, text: feedback.description, tone: feedback.urgency === "urgente" || feedback.urgency === "critique" ? "red" as Tone : "amber" as Tone, kind: "Blocage" }));
    const reserveAlerts = siteData.reserves.filter((reserve) => reserve.status !== "LEVEE").map((reserve) => ({ id: reserve.id, title: reserve.title, text: reserve.description ?? reserve.status, tone: reserve.priority === "URGENTE" ? "red" as Tone : "amber" as Tone, kind: "Reserve" }));
    const infoAlerts = siteData.infoRequests.filter((request) => request.status !== "traitee").map((request) => ({ id: request.id, title: request.subject, text: request.message, tone: "amber" as Tone, kind: "Information" }));
  ...
  return (
    <div className="min-h-dvh bg-slate-50 pb-[calc(5rem+var(--safe-bottom))] text-slate-900">
      ...
      {currentTask ? <TaskDrawer actionMessage={actionMessage} chantier={currentTaskChantier} close={() => setActiveTask(null)} completeTask={completeTask} consignes={siteData.consignes} documents={taskDocuments(currentTask, fieldDocuments)} drawerTab={drawerTab} entries={siteData.timeEntries.filter((entry) => entry.task_id === currentTask.id)} feedbacks={siteData.feedbacks} photoFile={photoFile} remarkText={remarkText} saving={savingAction} setDrawerTab={setDrawerTab} setPhotoFile={setPhotoFile} setRemarkText={setRemarkText} setSignalComment={setSignalComment} setSignalType={setSignalType} setTimeComment={setTimeComment} setTimeHours={setTimeHours} signalComment={signalComment} signalType={signalType} submitPhoto={submitPhoto} submitRemark={submitRemark} submitSignal={submitSignal} submitTime={submitTime} task={currentTask} timeComment={timeComment} timeHours={timeHours} /> : null}
    </div>
  );
}

function Badge({ children, tone = "neutral" }: { children: ReactNode; tone?: Tone }) {
  const tones: Record<Tone, string> = { neutral: "border-slate-200 bg-slate-50 text-slate-700", blue: "border-blue-200 bg-blue-50 text-blue-700", green: "border-emerald-200 bg-emerald-50 text-emerald-700", amber: "border-amber-200 bg-amber-50 text-amber-700", red: "border-red-200 bg-red-50 text-red-700" };
...
function Metric({ icon, title, value }: { icon: ReactNode; title: string; value: string }) { return <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3"><div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">{icon}{title}</div><div className="mt-2 line-clamp-2 text-sm font-semibold text-slate-950">{value}</div></div>; }
function InfoLine({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2"><div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">{label}</div><div className="mt-1 break-words text-sm font-semibold text-slate-900">{value}</div></div>; }
...
function Block({ children, empty, title }: { children: ReactNode; empty: string; title: string }) { const hasChildren = Array.isArray(children) ? children.some(Boolean) : Boolean(children); return <div className="mt-5 space-y-3"><div className="text-sm font-semibold text-slate-950">{title}</div>{hasChildren ? children : <Empty>{empty}</Empty>}</div>; }
function DocumentBlock({ documents, empty, title }: { documents: IntervenantDocument[]; empty: string; title: string }) { return <Block title={title} empty={empty}>{documents.slice(0, 8).map((document) => <div key={document.id} className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3"><FileText className="mt-0.5 h-4 w-4 shrink-0 text-blue-700" /><div><div className="text-sm font-semibold text-slate-950">{document.title ?? document.file_name ?? "Document"}</div><div className="mt-1 text-xs text-slate-500">{compactText(document.category, document.document_type) || "Document chantier"}</div></div></div>)}</Block>; }
