...
function SignalCard({ signal, onApplied }: { signal: CocoLearningSignal; onApplied: (signal: CocoLearningSignal, message: string) => void }) {
  const immediate = signal.kind === "immediate";
  const [decision, setDecision] = useState(signal.proposedAction);
  const [plan, setPlan] = useState<CocoImprovementPlan | null>(null);
  const [regenerating, setRegenerating] = useState(false);
  const [applying, setApplying] = useState(false);
  const [actionError, setActionError] = useState<string | null>(null);
  const canApply = Boolean(signal.chantierId && signal.sourceId);

  async function regenerate() {
    setRegenerating(true);
    setActionError(null);
    try {
      const result = await regenerateCocoImprovementProposal(signal, decision);
  ...
  async function apply() {
    ...
    setActionError(null);
    try {
      await applyCocoImprovementAction({ signal, decisionText: decision, plan });
      const message = signal.category === "materiel"
        ? "Décision appliquée : demande créée dans Achats et approvisionnements et confirmation publiée dans le fil chantier."
        : "Décision appliquée et publiée dans le fil chantier.";
      onApplied(signal, message);
    } catch (reason) {
  ...
  return <article className={`rounded-2xl border p-4 ${immediate ? "border-red-200 bg-red-50/60" : "border-slate-200 bg-white"}`}>
    ...
    <div className={`mt-3 rounded-xl p-3 ${immediate ? "bg-white" : "bg-blue-50"}`}>
      ...
      />
      {plan ? <div className="mt-2 rounded-lg bg-emerald-50 px-3 py-2 text-xs leading-5 text-emerald-800"><strong>COCO appliquera :</strong> {plan.actionType === "create_purchase_request" ? "création d'une demande dans Achats et approvisionnements, puis confirmation dans le fil chantier" : "publication de la décision dans le fil chantier"}.</div> : null}
      {actionError ? <div className="mt-2 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-700">{actionError}</div> : null}
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        <button type="button" onClick={() => void regenerate()} disabled={regenerating || applying || !decision.trim() || !signal.chantierId} className="inline-flex items-center justify-center gap-2 rounded-lg border border-blue-200 bg-white px-3 py-2.5 text-sm font-semibold text-blue-700 hover:bg-blue-50 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${regenerating ? "animate-spin" : ""}`} />{regenerating ? "Régénération…" : "Régénérer une proposition"}</button>
...
export default function CocoImprovementPage() {
  ...
  const immediate = useMemo(() => signals.filter((signal) => signal.kind === "immediate"), [signals]);
  const improvements = useMemo(() => signals.filter((signal) => signal.kind !== "immediate"), [signals]);
  const enabledCount = useMemo(() => Object.values(settings.sources).filter(Boolean).length, [settings.sources]);
  function handleApplied(signal: CocoLearningSignal, message: string) { setSignals((current) => current.filter((item) => item.id !== signal.id)); setNotice(message); }
