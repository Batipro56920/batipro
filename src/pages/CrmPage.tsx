...
export default function CrmPage({ section = "dashboard" }: Props) {
  ...
  const chantierPathByQuoteId = useMemo(() => {
    const paths = new Map<string, string>();
    for (const project of projects) {
      for (const quote of project.quotes) {
        const chantierId =
          quote.chantier_id ??
          project.chantiers.find((chantier) => chantier.crm_quote_id === quote.id)?.id ??
          (project.chantiers.length === 1 ? project.chantiers[0]?.id : null);
        if (chantierId) paths.set(quote.id, `/chantiers/${chantierId}`);
      }
    }
    return paths;
  }, [projects]);
  ...
  return (
    <div className="bt-page">
      <CrmHeader section={section} onRefresh={refresh} onCreateProspect={() => setModal("prospect")} onCreateQuote={createDraftQuoteAndOpen} />
      ...
      {loading ? (
        ...
        <CrmProspectsSection rows={data.prospects} users={data.users} query={query} setQuery={setQuery} onCreate={() => setModal("prospect")} onConvert={(row) => submitSafely(async () => convertProspectToClient(row))} onStatus={(row, statut) => submitSafely(async () => updateCrmProspect(row.id, { statut }))} onTask={(row) => submitSafely(async () => createCrmTask({ prospect_id: row.id, type: "relance", titre: `Relancer ${entityLabel(row)}`, due_at: new Date().toISOString() }))} onCreateAppointment={openAppointmentModal} onCreateQuote={createDraftQuoteAndOpen} onUpdate={(row, patch) => submitSafely(async () => updateCrmProspect(row.id, patch))} />
      ) : section === "clients" ? (
        <CrmClientsSection rows={data.clients} chantiers={data.chantiers} quotes={data.quotes} invoices={data.invoices} documents={data.documents} query={query} setQuery={setQuery} focusedClientId={focusedClientId} onFocusedClientClear={clearFocusedClientParam} onCreate={() => setModal("client")} />
      ) : section === "quotes" ? (
        <CrmQuotesSection rows={data.quotes} prospectById={prospectById} clientById={clientById} opportunityById={opportunityById} users={data.users} projectPathByQuoteId={projectPathByQuoteId} chantierPathByQuoteId={chantierPathByQuoteId} onCreate={createDraftQuoteAndOpen} onStatus={(row, statut) => submitSafely(async () => updateCrmQuote(row.id, { statut }))} onTransform={transformQuote} onPdf={downloadQuote} />
      ) : section === "invoices" ? (
