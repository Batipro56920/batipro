...
function taskDocuments(task: IntervenantTask, documents: IntervenantDocument[]) {
  ...
  const words = normalizeText(compactText(task.titre, task.zone_nom, task.lot, task.corps_etat)).split(/\s+/).filter((word) => word.length > 3);
...
export default function EmployeeFieldPortalV2Page() {
  ...
  const alerts = useMemo<AlertItem[]>(() => {
    ...
    const materielAlerts = siteDatas.flatMap((data) => data.materiels.filter((row) => OPEN_MATERIEL_STATUSES.has(row.statut)).map((row) => ({ id: row.id, label: normalizeText(row.titre).includes("materiaux") ? "Materiaux" : "Materiel", title: row.titre, text: compactText(row.task_titre, row.commentaire, row.admin_commentaire) || "Demande en attente cote admin.", tone: materielTone(row) })));
  ...
  function submitSignal(event: FormEvent<HTMLFormElement>) {
    ...
    const action = signalType === "materiel" || signalType === "materiaux"
      ? () => intervenantMaterielCreate(token, { chantier_id: task.chantier_id, task_id: task.id, titre: signalType === "materiaux" ? `Manque materiaux - ${task.titre}` : `Manque materiel - ${task.titre}`, commentaire: comment || null }).then(() => undefined)
...
function TimeView({ onOpenTask, selectedChantier, taskItems, today }: { onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void; selectedChantier: IntervenantChantier | null; taskItems: TaskItem[]; today: string }) {
...
function TaskPeriod({ items, onOpenTask, title }: { items: TaskItem[]; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void; title: string }) {
...
function TaskList({ empty, items, onOpenTask }: { empty: string; items: TaskItem[]; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void }) {
...
function TaskCard({ item, onOpenTask }: { item: TaskItem; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void }) {
...
function TimeGroup({ items, onOpenTask, title }: { items: TaskItem[]; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void; title: string }) {
...
function TimeTaskRow({ item, onOpenTask }: { item: TaskItem; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void }) {
