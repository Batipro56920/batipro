...
function taskExtra(task: IntervenantTask): TaskExtra {
  return task as TaskExtra;
}
...
function documentSearchText(document: IntervenantDocument) {
  return normalize(compact(document.title, document.file_name, document.category, document.document_type));
}
...
async function loadSiteData(token: string, chantierId: string) {
  ...
  return {
    data: {
      ...
      informationRequests: informationRequests.data,
...
export default function EmployeePortalV2Page() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      if (!candidateToken) {
        if (alive) {
          setToken("");
          setBootError("Colle ton lien d'acces intervenant pour ouvrir le portail terrain.");
  ...
  const alerts = useMemo(() => buildAlerts(dataByChantier, planDocuments), [dataByChantier, planDocuments]);
  ...
  function submitPhoto(event: FormEvent<HTMLFormElement>) {
    ...
    void runTaskAction(async () => {
      const feedback = await intervenantTerrainFeedbackCreate(token, { chantier_id: task.chantier_id, category: "photo", urgency: "normale", title: `Photo - ${task.titre}`, description: "Photo ajoutee depuis le portail terrain." });
  ...
  function completeTask() {
    if (!token || !activeTask) return;
    const task = activeTask;
    void runTaskAction(() => intervenantUpdateTaskStatus(token, task.id, "FAIT"), "Tache marquee terminee.");
  }
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 px-4 pt-[var(--safe-top)] backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 py-3">
          <div className="min-w-0">
            <div className="text-[11px] font-semibold uppercase text-blue-700">Portail employe V2</div>
...
function buildAlerts(dataByChantier: Record<string, SiteData>, planDocuments: IntervenantDocument[]) {
  ...
  const feedbackAlerts = siteDatas.flatMap((data) => data.feedbacks
    ...
    .map((feedback) => ({ id: feedback.id, label: "Blocage", title: feedback.title, text: feedback.description, tone: feedback.urgency === "urgente" || feedback.urgency === "critique" ? "red" as Tone : "amber" as Tone })));
  const reserveAlerts = siteDatas.flatMap((data) => data.reserves
    .filter((reserve) => reserve.status !== "LEVEE")
    .map((reserve) => ({ id: reserve.id, label: "Reserve", title: reserve.title, text: reserve.description ?? reserve.status, tone: reserve.priority === "URGENTE" ? "red" as Tone : "amber" as Tone })));
  const materielAlerts = siteDatas.flatMap((data) => data.materiels
    .filter((row) => OPEN_MATERIEL_STATUSES.has(row.statut))
    .map((row) => ({ id: row.id, label: normalize(row.titre).includes("materiaux") ? "Materiaux" : "Materiel", title: row.titre, text: compact(row.task_titre, row.commentaire, row.admin_commentaire) || "Demande en attente cote admin.", tone: row.statut === "validee" ? "blue" as Tone : "amber" as Tone })));
  const infoAlerts = siteDatas.flatMap((data) => data.informationRequests
    .filter((request) => request.status !== "traitee")
    .map((request) => ({ id: request.id, label: "Information", title: request.subject, text: request.message, tone: "amber" as Tone })));
  const docAlerts = planDocuments.slice(0, 1).map((document) => ({ id: document.id, label: "Document", title: document.title ?? document.file_name ?? "Document important", text: compact(document.category, document.document_type) || "Document chantier", tone: "blue" as Tone }));
...
function TasksView({ laterTasks, onOpenTask, todayTasks, weekTasks }: { laterTasks: TaskContext[]; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void; todayTasks: TaskContext[]; weekTasks: TaskContext[] }) {
  return <Surface title="Taches"><TaskPeriod title="Aujourd'hui" items={todayTasks} onOpenTask={onOpenTask} /><TaskPeriod title="Cette semaine" items={weekTasks} onOpenTask={onOpenTask} /><TaskPeriod title="Plus tard" items={laterTasks} onOpenTask={onOpenTask} /></Surface>;
}
        ...
        <div className="h-[calc(92dvh-9rem)] overflow-y-auto px-4 py-4 sm:h-[calc(100dvh-11rem)]">
          {props.drawerTab === "infos" ? (
            <div className="space-y-3">
              ...
              <Block title="Temps saisis" empty="Aucun temps saisi sur cette tache.">{props.entries.slice(0, 5).map((entry) => <NoteRow key={entry.id} title={formatHours(entry.duration_hours)} text={compact(formatDate(entry.work_date), entry.note)} tone="blue" label="Temps" />)}</Block>
              <Block title="Materiel et materiaux" empty="Aucune demande liee.">{props.materiels.map((row) => <NoteRow key={row.id} title={row.titre} text={compact(row.commentaire, row.admin_commentaire)} tone={row.statut === "validee" ? "blue" : "amber"} label={row.statut} />)}</Block>
...
function TaskPeriod({ items, onOpenTask, title }: { items: TaskContext[]; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void; title: string }) {
  const groups = new Map<string, { chantier: IntervenantChantier; items: TaskContext[] }>();
...
function TaskList({ empty, items, onOpenTask }: { empty: string; items: TaskContext[]; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void }) {
...
function TaskCard({ item, onOpenTask }: { item: TaskContext; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void }) {
...
function SmallTaskGroup({ chantier, data, onOpenTask, tasks, title }: { chantier: IntervenantChantier; data: SiteData; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void; tasks: IntervenantTask[]; title: string }) {
...
function TimeGroup({ items, onOpenTask, title }: { items: TaskContext[]; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void; title: string }) {
  return <div className="mt-4"><div className="text-sm font-semibold text-slate-950">{title}</div><div className="mt-2 space-y-2">{items.length ? items.map((item) => <TimeTaskRow key={`${title}-${item.task.id}`} item={item} onOpenTask={onOpenTask} />) : <Empty>Aucune tache dans ce groupe.</Empty>}</div></div>;
}

function TimeTaskRow({ item, onOpenTask }: { item: TaskContext; onOpenTask: (task: IntervenantTask, tab?: DrawerTab) => void }) {
