...
export default function ChantierVisitesPage() {
  ...
  async function loadPage() {
    ...
    try {
      ...
      setError(err?.message ?? "Erreur chargement page visites.");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (!id) {
    return <div className="text-sm text-red-700">Chantier introuvable.</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-[0.2em] text-slate-500">Livrables</div>
          <h1 className="text-xl font-semibold text-slate-900">Visites de chantier</h1>
          <p className="text-sm text-slate-500">{chantier?.nom ?? "Chantier"}</p>
        </div>
