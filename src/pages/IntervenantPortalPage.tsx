    ...
    <div className="min-h-screen bg-slate-100 px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] text-slate-900 sm:px-4 lg:px-6">
      <div className="mx-auto max-w-7xl md:grid md:grid-cols-[18rem_minmax(0,1fr)] md:gap-5">
        ...
        <main className="min-w-0 space-y-3 md:space-y-4">
          ...
          {chantiers.length > 0 ? <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm md:hidden"><div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Chantier</div>{chantiers.length === 1 ? <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-800">Chantier : {activeChantier?.nom ?? chantiers[0]?.nom ?? "-"}</div> : <div className="mt-3 space-y-2"><div className="rounded-2xl border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-medium text-blue-700">Chantier actif : {activeChantier?.nom ?? "Choisir un chantier"}</div><select className="w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm" value={selectedChantierId} onChange={(e) => chooseChantier(e.target.value)}>{chantiers.map((chantier) => <option key={chantier.id} value={chantier.id}>{chantier.nom}</option>)}</select></div>}</section> : null}
          {selectedChantierId ? <nav className="no-scrollbar flex gap-2 overflow-x-auto rounded-3xl border border-slate-200 bg-white p-2 shadow-sm md:hidden">{(["temps","taches","planning","documents","materiel","messages"] as PortalTab[]).map((tab) => <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={["shrink-0 rounded-full px-3 py-2 text-sm font-medium", activeTab === tab ? "bg-blue-600 text-white" : "border border-slate-200 text-slate-700"].join(" ")}>{tab === "taches" ? "Taches" : tab.charAt(0).toUpperCase() + tab.slice(1)}</button>)}</nav> : null}
