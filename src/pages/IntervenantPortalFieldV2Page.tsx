...
function taskPhotoCount(task: IntervenantTask, feedbacks: IntervenantTerrainFeedback[]) {
  ...
  return feedbacks
    .filter((feedback) => feedback.title.toLowerCase().includes(title) || feedback.description.toLowerCase().includes(title))
    ...
    <>
      <Card>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          ...
          <Metric icon={<ListChecks className="h-4 w-4" />} title="Taches jour" value={String(todayTasks.length)} />
          <Metric icon={<Clock3 className="h-4 w-4" />} title="Temps saisi" value={formatHours(todayHours)} />
      ...
      <Card>
        ...
        <div className="mt-3 space-y-2">
          {alerts.length ? alerts.map((alert) => <NoteRow key={`${alert.kind}-${alert.id}`} label={alert.kind} text={alert.text} title={alert.title} tone={alert.tone} />) : <Empty>Aucune alerte bloquante.</Empty>}
        </div>
      </Card>
    </>
  );
