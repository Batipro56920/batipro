...
function taskDocuments(task: IntervenantTask, documents: IntervenantDocument[]) {
  ...
  const taskWords = normalizeText(compactText(task.titre, task.zone_nom, task.lot, task.corps_etat))
    .split(/\s+/)
...
function taskPhotoCount(task: IntervenantTask, feedbacks: IntervenantTerrainFeedback[]) {
  ...
  return feedbacks
    .filter((feedback) => normalizeText(`${feedback.title} ${feedback.description}`).includes(title))
...
export default function IntervenantPortalV2CompletePage() {
  ...
  const alerts = useMemo<AlertItem[]>(() => {
    ...
    const docAlerts = planDocuments.slice(0, 1).map((document) => ({
      id: document.id,
      label: "Document",
      title: document.title ?? document.file_name ?? "Document important",
      text: compactText(document.category, document.document_type) || "Document chantier",
      tone: "blue" as Tone,
    }));
  ...
  return (
    <div className="min-h-dvh bg-slate-50 pb-[calc(5.5rem+var(--safe-bottom))] text-slate-950">
      ...
      <main className="mx-auto max-w-5xl space-y-3 px-3 py-3 sm:px-4">
        ...
        {activeTab === "chantiers" ? (
          <ChantiersView
            chantiers={chantiers}
            dataByChantier={dataByChantier}
            onOpenTask={openTask}
            onSelect={selectChantier}
    ...
    <>
      <Card>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          <Metric icon={<Building2 className="h-4 w-4" />} title="Chantier" value={selectedChantier?.nom ?? "Non selectionne"} />
      ...
      <Card>
        ...
        <div className="mt-3 space-y-2">
          {alerts.length ? alerts.map((alert) => <NoteRow key={`${alert.label}-${alert.id}`} label={alert.label} text={alert.text} title={alert.title} tone={alert.tone} />) : <Empty>Aucune alerte bloquante.</Empty>}
        </div>
      </Card>
    </>
  );
    ...
    <>
            ...
            return (
              <button key={chantier.id} type="button" onClick={() => props.onSelect(chantier.id)} className={`w-full rounded-lg border px-3 py-3 text-left transition ${active ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-slate-50 hover:border-blue-200"}`}>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  <InfoLine label="Progression" value={`${chantierProgress(data, chantier)} %`} />
                  <InfoLine label="Restantes" value={String(remaining)} />
                  <InfoLine label="Docs" value={String(data.documents.filter(isFieldDocument).length)} />
                </div>
              </button>
            );
          })}
      ...
      {props.selectedChantier ? (
        <Card>
          ...
          <Block title="Consignes et contraintes acces" empty="Aucune consigne visible.">
          ...
          <Block title="Taches chantier" empty="Aucune tache visible.">
            ...
            <SmallTaskGroup title="Cette semaine" tasks={props.selectedData.tasks.filter((task) => {
              ...
              return !!date && date >= today && date <= weekEnd && !isTaskDone(task);
...
function TimeView({ onOpenTask, selectedChantier, taskItems, today }: { onOpenTask: (task: IntervenantTask) => void; selectedChantier: IntervenantChantier | null; taskItems: TaskItem[]; today: string }) {
  ...
  return (
    <Card>
      ...
      <TimeGroup title="Taches recentes" items={recentItems} onOpenTask={onOpenTask} />
      <TimeGroup title="Taches du jour" items={todayItems} onOpenTask={onOpenTask} />
      <TimeGroup title="Chantier actuel" items={currentSiteItems} onOpenTask={onOpenTask} />
    </Card>
  );
}
  ...
  return (
    <div className="mt-4">
      ...
      <div className="mt-2 space-y-2">
        {items.length ? items.map((item) => <TimeTaskRow key={`${title}-${item.task.id}`} item={item} onOpenTask={onOpenTask} />) : <Empty>Aucune tache dans ce groupe.</Empty>}
      </div>
    </div>
  );
}
...
function TimeTaskRow({ item, onOpenTask }: { item: TaskItem; onOpenTask: (task: IntervenantTask) => void }) {
  ...
  return (
    <button type="button" onClick={() => onOpenTask(item.task)} className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 text-left">
...
function TaskPeriod({ items, onOpenTask, title }: { items: TaskItem[]; onOpenTask: (task: IntervenantTask) => void; title: string }) {
  ...
  items.forEach((item) => {
    const group = groups.get(item.chantier.id) ?? { chantier: item.chantier, items: [] };
...
function Card({ children }: { children: ReactNode }) {
  return <section className="rounded-lg border border-slate-200 bg-white p-3 shadow-[0_1px_2px_rgba(15,23,42,0.04),0_10px_30px_rgba(15,23,42,0.04)] sm:p-4">{children}</section>;
}
