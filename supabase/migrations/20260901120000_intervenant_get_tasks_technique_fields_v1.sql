-- intervenant_get_tasks oubliait de remonter description_technique/materiaux/
-- contraintes/points_controle alors que ces colonnes existent sur
-- chantier_tasks depuis 20260403000100 et 20260515171000. Utile pour la fiche
-- "Chantier" du portail ouvrier (dossier/infos pratiques par tâche).

drop function if exists public.intervenant_get_tasks(text, uuid);
create function public.intervenant_get_tasks(p_token text, p_chantier_id uuid)
returns table (
  id uuid,
  chantier_id uuid,
  titre text,
...
begin
  ...
  select
    ...
    nullif(btrim(coalesce(t.description_technique, '')), '') as description_technique,
    nullif(btrim(coalesce(t.materiaux, '')), '') as materiaux,
    nullif(btrim(coalesce(t.contraintes, '')), '') as contraintes,
    nullif(btrim(coalesce(t.points_controle, '')), '') as points_controle,
